    ...
    >
      <MaxContainer>
        <div className="flex justify-between max-w-[130rem] mx-auto px-[7rem] sm:px-[2.5rem]">
          <Link
            href={'/'}
            className={`${pathName.includes('/registration') ? 'sm:mx-auto' : 'sm:mx-0'}`}
          >
            <Image
              width={87}
              height={37}
              alt="logo"
              className="w-[7rem]"
              src={pathName !== '/' || scrollY > 100 ? '/common/logo_ii.svg' : '/home/offa_logo.svg'}
            />
          </Link>
          ...
          {!pathName.includes('/registration') && <Hamburger open={open} scrollY={scrollY} toggleNav={toggleNav} />}
        </div>
      </MaxContainer>
    </nav>
  );
